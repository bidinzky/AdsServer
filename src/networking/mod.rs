...
impl<'a> ToPlcConn for (&'a str, &'a str) {
    fn try_into_plc_conn(&self) -> Option<[u8; 8]> {
        let net_id: Vec<_> = self
            ...
            .map(|x| u8::from_str_radix(x, 10).unwrap())
            .collect();
        let port = u16::from_str_radix(self.1, 10).unwrap();
        let mut d = [0u8; 8];
        for i in 0..6 {
            d[i] = net_id[i];
        }
...
impl fmt::Debug for SimpleSocket {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "SimpleSocket {{ {:?}, {:?}, {:?}, {:?} }}", self.alive, self.queue, self.target, self.source)
    }
}

impl SimpleSocket {
    pub fn new<A: net::ToSocketAddrs, B: ToPlcConn, C: ToPlcConn>(
        a: A,
        target: B,
        source: C,
                    ...
                    match stream.read_exact(&mut b) {
                        Ok(_) => {
                            ...
                            if map.contains_key(&tcp.header.inv_id) {
                                ...
                                thread_c.lock().unwrap().broadcast(r.clone());
                                let mut v = Vec::new();
                                r.into_writer(&mut v);
